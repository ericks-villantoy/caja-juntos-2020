...
public class CorrectBrowserCacheHandlerFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        resourcesCanChange = BooleanUtils.toBoolean(System.getProperty("correctBrowserCacheHandlerFilter.resourcesCanChange", "false"));
    }
    ...
    private String retrieveHashForRequestURIIfValidThenUpdateMapElseHandleRequest(HttpServletRequest httpRequest, HttpServletResponse httpResponse, FilterChain chain) throws IOException, ServletException {
        ...
        if (BooleanUtils.isTrue(responseWrapper.requestOK)) {
            ...
            String hash = convertFromBytesToBase64RFC4648(responseWrapper.hasher.hash().asBytes()).substring(0, 6); // we strip the last 2 "=" character that will still be there as the hash is 32bits, which makes 4 bytes, so 6 Base64 chars with 2 "=" padding at the ead
